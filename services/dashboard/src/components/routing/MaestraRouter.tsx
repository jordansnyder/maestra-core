...
export function MaestraRouter() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full font-mono text-fg items-center justify-center bg-surface-0">
  ...
  return (
    <div className="flex flex-col h-full font-mono text-fg">
      {/* Toolbar */}
      <div className="flex items-center justify-between px-5 py-3 border-b border-edge bg-surface-1">
        ...
        {/* View switcher */}
        <div className="flex gap-0.5 bg-surface-1 rounded-lg p-[3px]">
          {VIEWS.map((v) => (
            <button
              key={v.id}
          ...
          <button
            ...
            className={`rounded-md px-3 py-1 text-[10px] font-mono cursor-pointer transition-colors ${
              showPresets
                ...
                : 'bg-surface-1 border border-edge text-fg-muted hover:text-fg'
            }`}
          >
            Presets
          </button>
      ...
      {showPresets && (
        <div className="px-5 py-3 border-b border-edge bg-surface-1 flex items-center gap-4 flex-wrap">
          {/* Existing presets */}
          {presets.map((preset) => (
            <div
              key={preset.id}
              className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-[11px] font-mono border ${
                preset.is_active
                  ...
                  : 'bg-surface-1 border-edge text-fg-muted'
              }`}
            >
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="bg-surface-1 border border-edge rounded-md px-2 py-1 text-[11px] font-mono text-fg placeholder-fg-subtle focus:outline-none focus:border-accent w-40"
            />
            <button
      ...
      {/* Status bar */}
      <div className="flex items-center justify-between px-5 py-1.5 border-t border-edge bg-surface-1 text-[10px] text-fg-subtle">
