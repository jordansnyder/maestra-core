-- Migration 014: OSC Mappings
-- Adds osc_mappings table for web-managed OSC address → entity state mappings
-- with hot-reload support via NATS
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'update_osc_mappings_updated_at'
    ) THEN
